...
class StudioAIProvider(Document):
	def validate(self):
		self.derive_routing()
	...
	def unique_prefix(self, base: str) -> str:
		...
		while frappe.db.exists("Studio AI Provider", {"route_prefix": prefix, "name": ["!=", self.name]}):
			prefix, suffix = f"{base}-{suffix}", suffix + 1
		return prefix

	def resolved_key(self) -> str | None:
		"""This provider's stored key (or OAuth credential blob), if any."""
	...
	def on_trash(self):
		if models := frappe.get_all("Studio AI Model", filters={"provider": self.name}, pluck="name"):
			frappe.throw(
