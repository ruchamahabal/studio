...
chat-completions chunk shape the rest of studio.ai already reads: llm.py hands
...
def verify_credential(raw: str) -> dict:
	...
	try:
		...
		from studio.ai.presets import readable_error
...
def stored_credential(provider: str) -> dict:
	raw = frappe.get_cached_doc("Studio AI Provider", provider).resolved_key()
	if not raw:
...
def save_credential(provider: str, cred: dict) -> None:
	...
	set_encrypted_password("Studio AI Provider", provider, json.dumps(cred), "api_key")
...
def log(message: str) -> None:
	...
	logger = frappe.logger("studio.ai.codex")
