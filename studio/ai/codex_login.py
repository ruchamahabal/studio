...
from studio.ai.codex import CLIENT_ID, ORIGINATOR, TOKEN_URL, CodexError, account_id_from_tokens
...
class CallbackHandler(BaseHTTPRequestHandler):
	def do_GET(self):
		...
		if outcome is None:
			self.respond(400, "This sign-in attempt is stale. Return to Studio and try again.")
			return
...
def complete_captured(site: str, state: str, code: str) -> tuple[bool, str] | None:
	...
	try:
		...
		try:
			...
			return True, "Connected. You can close this tab and return to Studio."
		except Exception as e:
			save_login(state, {"status": "failed", "message": str(e)})
			return False, "Sign-in failed. Return to Studio and try again."
	finally:
		frappe.destroy()


PAGE = """<!doctype html>
<html><head><meta charset="utf-8"><title>Studio</title></head>
