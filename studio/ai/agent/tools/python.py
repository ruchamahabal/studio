...
RESULT_LIMIT = 6000
SAVEPOINT = "studio_ai_run_python"

# Beyond safe_exec's own rules: no raw SQL (server scripts may read any table,
# including auth internals — Studio users aren't script authors) and no direct
...
def sandbox_globals() -> dict:
	...
	# permissions: the agent sees exactly what the person driving it could open in Desk.
...
def run_python(ctx, args: dict) -> str:
	...
	_locals = {"page_id": ctx.target_page_id or ctx.page_id, "result": None}
...
run_python_tool = Tool(
	...
	description=(
		...
		"helpers, frappe.session.user, and `page_id` (the working page). No imports, no raw "
