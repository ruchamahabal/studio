"""The model registry: Studio AI Provider / Studio AI Model rows + live metadata.
...
deploy; `seed_providers_and_models` seeds the shipped shortlist on migrate.

Everything factual about an OpenRouter model — context window, vision support —
is fetched from OpenRouter's public models API and cached, so it never goes
stale. The values stored on the row are the offline fallback, and the only
source for every other provider.
"""
...
logger = frappe.logger("studio.ai.models")
	...
	try:
		providers = {
			...
			for p in frappe.get_all(
				"Studio AI Provider",
				filters={"enabled": 1},
				fields=["name", "route_prefix", "litellm_provider", "api_base"],
			)
		}
		rows = frappe.get_all(
			"Studio AI Model",
			filters={"enabled": 1},
			fields=["name", "label", "provider", "supports_vision", "max_tokens"],
...
def lookup_metadata(qualified_name: str, model_id: str) -> dict:
	...
	if qualified_name.startswith("codex/"):
		from studio.ai.codex import model_metadata
...
class ModelRegistry:
	...
	@classmethod
	def is_vision_capable(cls, model_name: str) -> bool:
	...
	@classmethod
	def get_default(cls, model_or_provider: str | None = None) -> str:
		"""Resolve a selection to a model name. A known model passes through;
		anything else falls back to the first configured model."""
		if model_or_provider and cls.find(model_or_provider):
			return model_or_provider
		catalog = cls.catalog()
		return catalog[0]["name"] if catalog else (model_or_provider or FALLBACK_MODEL["name"])


@frappe.whitelist()
@has_page_write_perm()
def get_ai_models() -> list:
	"""The shape the model picker consumes. `ready` says whether this model can
	actually be called: its provider's own key, or the shared one in Studio
	Settings. Computed here rather than in the cached row loader, so key state is
	never held in redis, and so the picker can say a model is unusable BEFORE
	someone writes a prompt for it."""
	from studio.ai.llm import provider_api_key

	fallback = bool(frappe.get_single("Studio Settings").get_password("ai_api_key", raise_exception=False))
	return [
		{
			"id": m["name"],
			"label": m["label"],
			"provider": m["provider"],
			"vision_capable": m["vision"],
			"ready": bool(provider_api_key(m)) or fallback,
		}
		for m in ModelRegistry.catalog()
	]
