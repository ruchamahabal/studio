...
def get_ai_models() -> list:
	...
	actually be called: its provider carries a key, or is a self-hosted gateway
	that needs none. Computed here rather than in the cached row loader, so key
	state is never held in redis, and so the picker can say a model is unusable
	BEFORE someone writes a prompt for it."""
	...
	return [
		{
			...
			"ready": bool(provider_api_key(m) or m.get("api_base")),
		}
