<template>
	<component :is="block.getProp('tag')" ref="component" :key="editor" class="__text_block__">
...
import { Ref, computed, inject, onBeforeUnmount, ref, watch } from "vue"
...
const textContent = computed(() => {
	let innerHTML = props.block.getProp("text")
...
const showEditor = computed(() => {
	return !props.block.hasChildren()
})
			...
			if (canvasStore.activeCanvas?.isSelected(props.block) && !blockController.multipleBlocksSelected()) {
				editor.value = new Editor({
					...
					onUpdate({ editor }) {
						...
						if (props.block.getProp("text") === innerHTML) {
							return
						}
						dataChanged.value = true
						// if (!getDynamicContent()) {
						props.block.setProp("text", innerHTML)
						// }
					},
					autofocus: false,
					injectCSS: false,
