 ...
 * These are provided at runtime by Studio's host application via window.__studio_shared__.
 */
const SHARED_DEPS = ["vue", "vue-router", "frappe-ui"]

function isSharedDep(source) {
	return SHARED_DEPS.some((dep) => source === dep || source.startsWith(dep + "/"))
}
...
/**
 * Vite plugin that externalizes shared deps and rewrites them in the output
 * to load from window.__studio_shared__.
 *
 * Two phases:
 * 1. resolve: marks shared deps as external (rollup leaves their import statements)
 * 2. renderChunk: replaces `import {...} from "vue"` with
 *    `const {...} = window.__studio_shared__["vue"]` in the final output
 */
function studioExternalsPlugin() {
	return {
		name: "studio-externals",
		enforce: "pre",
		resolveId(source) {
			if (isSharedDep(source)) {
				return { id: source, external: true }
			}
			return null
		},
		renderChunk(code) {
			// Replace ES import statements for shared deps with window.__studio_shared__ refs
			for (const dep of SHARED_DEPS) {
				const escaped = dep.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")

				// Match: import { X, Y } from "dep" or "dep/subpath"
				const importRegex = new RegExp(
					`import\\s*\\{([^}]+)\\}\\s*from\\s*["'](${escaped}(?:\\/[^"']*)?)["'];?`,
					"g",
				)
				code = code.replace(importRegex, (_, names, fullPath) => {
					const cleanNames = names.trim()
					return `const { ${cleanNames} } = window.__studio_shared__["${fullPath}"];`
				})

				// Match: import X from "dep" or "dep/subpath"
				const defaultImportRegex = new RegExp(
					`import\\s+(\\w+)\\s+from\\s*["'](${escaped}(?:\\/[^"']*)?)["'];?`,
					"g",
				)
				code = code.replace(defaultImportRegex, (_, name, fullPath) => {
					return `const ${name} = window.__studio_shared__["${fullPath}"];`
				})
			}
			return code
		},
...
async function buildEntry({ app, entryName, entryPath }) {
	...
	await build({
		...
		plugins: [vue(), studioExternalsPlugin()],
		build: {
