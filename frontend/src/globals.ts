...
export async function registerCustomVueComponents(app: App): Promise<CustomVueComponentMeta[]> {
	try {
		const { frappeRequest } = await import("frappe-ui")
		const { default: componentRegistry } = await import("@/data/components")
		const { default: Block } = await import("@/utils/block")
		...
		// Update __APP_COMPONENTS__ after registration
		window.__APP_COMPONENTS__ = app._context.components

		// Refresh Block.components with updated registry
